...
class Jurnal_model extends CI_Model {
	...
	function set_month_year($month, $year, $sign = '=')
	{
		...
		if($month !== '0')
		{
 		...
 		if($unit) $this->db->where("EXTRACT($unit FROM tanggal_jurnal) $sign", $val);
	}
...
*/
	...
	function get_data()
	{
		$this->db->select('jurnal.id, jurnal.tanggal_jurnal, jurnal.keterangan, jurnal.f_id AS f_id, f.nama AS f_name, jurnal.id_bukti,  jurnal_detail.item, jurnal_detail.akun_id, jurnal_detail.jurnal_id, akun.nama as account_name, akun.kelompok_akun_id, jurnal_detail.debit_kredit, jurnal_detail.nilai');
		$this->db->from('jurnal');
		...
		$this->db->order_by('jurnal.tanggal_jurnal', 'asc');
		$query = $this->db->get();
	...
	{
		$this->db->select('MONTH(jurnal.tanggal_jurnal) AS month, YEAR(jurnal.tanggal_jurnal) AS year, kelompok_akun.id AS kelompok_akun, jurnal_detail.debit_kredit, SUM(jurnal_detail.nilai) AS nilai');
		$this->db->from('jurnal');
		...
		$this->db->where('extract(year_month from jurnal.tanggal_jurnal) > extract(year_month from (date_sub(curdate(), interval 1 year)))');		
		$this->db->group_by(array('MONTH(jurnal.tanggal_jurnal)', 'YEAR(jurnal.tanggal_jurnal)', 'kelompok_akun.id', 'jurnal_detail.debit_kredit'));
		$query = $this->db->get();
		if ($query->num_rows() > 0)
		{
	...
	function fill_data()
	{
		$this->data = array(
			...
			'tanggal_jurnal' => $this->input->post('tanggal'),
