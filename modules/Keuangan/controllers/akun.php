<?php
class Akun extends CMS_Controller {
	...
	function index()
	{
        ...
        $crud->unset_texteditor(array('keterangan'));
    	$output = $crud->render();        
	...
	function add()
	{
		...
		$this->view($data['main_content'], $data);
	}
	...
	function detail_akun()
	{
		...
		$this->view('akun/detail_akun', $data,'Keuangan_buku_besar');
	}
	...
	function saldo_awal()
	{
		...
		$data['account_data'] = $this->akun_model->get_all_data();
		$this->view('akun/saldo_awal', $data,'Keuangan_saldo_awal');
	}
		...
		else
		{
			...
			else
			{
				...
				if($this->akun_model->set_saldo_awal())
				{
					...
					redirect('Keuangan/akun/saldo_awal');
				}
			}
		}
	}
